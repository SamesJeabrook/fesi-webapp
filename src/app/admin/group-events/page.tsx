    ...
    <ProtectedRoute requireRole={['admin']}>
      <div className={styles.container}>
        <div className={styles.header}>
          <div>
            <Typography variant="heading-2">Group Events</Typography>
            <Typography variant="body-medium" className={styles.subtitle}>
              Organize and manage group events with participating merchants
            </Typography>
          </div>
          <Button
            variant="primary"
            size="md"
            onClick={() => router.push('/admin/group-events/create')}
          >
            + Create Event
          </Button>
        </div>
        ...
        {events.length === 0 ? (
          <Card className={styles.emptyState}>
            <div className={styles.emptyContent}>
              ...
              <Typography variant="heading-4">No events yet</Typography>
              <Typography variant="body-medium" className={styles.emptyText}>
                Create your first group event to start inviting merchants and coordinating sales
              </Typography>
              <Button
                variant="primary"
                onClick={() => router.push('/admin/group-events/create')}
              ...
              return (
                <Card key={event.id} className={styles.eventCard}>
                  ...
                  <div className={styles.cardActions}>
                    <Link href={`/admin/group-events/${event.id}`}>
                      <Button variant="secondary" size="sm">
                        View Details
                      </Button>
                    </Link>
                    <Link href={`/admin/group-events/${event.id}/invite`}>
